...
<!--#######################-->
    ...
    <div class="timber-idTag"> 
        ...
        <?php echo htmlspecialchars($row['source'] ?? ''); ?> | <?php echo htmlspecialchars($row['title'] ?? ''); ?> - 
    ...
    <div class="btn timber-trash"><a 
        href="/actors/delete_timber.php?id=<?php echo $row['id']; ?>" 
...
<!-- make a fancy box with overlay and nice positioning -->
    ...
    <span class="timber-headerFloat"
    <!-- this lil button closes -->
    <button style="height: 30px; width: 75px;" onclick="showManageInput(<?php echo $row['id']; ?>)">
    Close
    </button>
    </span>
    ...
    <div style="tools-section record-keeper">

    <h1>Record Keeper</h1>
    <span class="branch">
        Log Yard Code: <?php echo htmlspecialchars($row['title']); ?>-
        <?php echo $prefix . $row['block_id']; ?>-
        <?php echo $row['order_index'] + 1; ?>
    </span>
    ...
    <div style="tools-section tags-keeper">

    

    <h1>Crata Suggestables</h1>
    <input 
    type="text" 
    ...
    ?>
<div id="leaf-container-<?php echo $row['id']; ?>" class="leaf-container"></div>
...
<!-- LEAF BUTTON (THIS IS WHAT ENABLES RECURSION) -->
    ...
    <a href="/actors/delete_timber.php?id=<?php echo $leaf['id']; ?>" 
...
<?php endwhile; ?>
    ...
    <!-- this lil button deletes -->
    <a href="/actors/delete_timber.php?id=<?php echo $row['id']; ?>" 
...
function handleTagAutocomplete(e, timberId) {
    ...
    fetch("/actors/search_tags.php?term=" + encodeURIComponent(value))
        .then(res => res.json())
        .then(data => {
            box.innerHTML = "";
...
function addLeafToUI(parentId, data) {
    ...
    console.log("ADDING LEAF", parentId, data, container);

    let div = document.createElement("div");
    div.innerText = "TEST LEAF: " + data.content;
